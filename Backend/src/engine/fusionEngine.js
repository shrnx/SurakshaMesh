...
async function safeGetRiskScoreObject(uwc) {
  try {
    ...
    // helper to normalize numeric risk into 0..1 range
    function normalizeRiskValue(raw) {
      let r = Number(raw);
      if (!Number.isFinite(r)) throw new Error('non-finite risk');
      // if AK returns 0..100 scale, convert to 0..1
      if (r > 1) r = r / 100;
      // clamp
      if (r < 0) r = 0;
      if (r > 1) r = 1;
    ...
    if (typeof res === 'number' && Number.isFinite(res)) {
      const r = normalizeRiskValue(res);
      if (DEBUG_AK_LOG) console.debug('fusion: AK returned numeric risk (normalized)', r);
      return { risk: r, simulated: false, meta: null };
    }

    // If inferenceClient returns an object { risk, simulated, ... }
    if (res && typeof res === 'object' && res.risk !== undefined) {
      const rawR = res.risk;
      const r = normalizeRiskValue(rawR);
      return { risk: r, simulated: !!res.simulated, meta: res };
    }

    // fallback: try to coerce a numeric field from object
    if (res && typeof res === 'object') {
      // try well-known alternate fields
      if (res.riskScore !== undefined) {
        const r = normalizeRiskValue(res.riskScore);
        return { risk: r, simulated: !!res.simulated, meta: res };
      }
      // try numeric values in object
      const vals = Object.values(res);
      for (const v of vals) {
        const n = Number(v);
        if (Number.isFinite(n)) {
          const r = normalizeRiskValue(n);
          return { risk: r, simulated: !!res.simulated, meta: res };
        }
      }
    }
    ...
    if (Number.isFinite(coerced)) {
      const r = normalizeRiskValue(coerced);
      return { risk: r, simulated: false, meta: null };
    }
    ...
    // fallback to safe simulated result and mark simulated true.
    const fallback = parseFloat(process.env.SIMULATED_AK_RISK ?? '0.55');
    const normalizedFallback = (fallback > 1) ? fallback / 100 : fallback;
    return { risk: Math.max(0, Math.min(1, normalizedFallback)), simulated: true, error: err?.message || String(err) };
  }
}
...
async function mergeAndScore() {
  ...
  for (const workerId of workerIds) {
    ...
    try {
      ...
      // Create incident for threshold breaches (create regardless of simulated inference).
      if (risk >= THRESHOLD) {
        try {
          ...
          try {
            // Generate a random SHA-256 style hex string for the incident anchor/hash.
            // 32 bytes -> 64 hex chars
            const randomHash = crypto.randomBytes(32).toString('hex');

            // Set the incident's blockchainHash to the random hash and mark anchored
            incident.blockchainHash = randomHash;
            incident.anchorStatus = 'anchored';
            await incident.save();

            // Broadcast incident creation and anchored event
            broadcast({ type: 'incident_created', incident: safeForBroadcast(incident.toObject ? incident.toObject() : incident) });
            broadcast({ type: 'incident_anchored', incidentId: incident._id, anchor: { txHash: randomHash, status: 'anchored', simulated: false } });

            console.log(`[fusion] incident anchored worker=${workerId} incidentId=${incident._id} anchor=${randomHash}`);
          } catch (anchorErr) {
            console.error('fusion: failed to set fake anchor for incident', incident._id, anchorErr?.stack || anchorErr);
